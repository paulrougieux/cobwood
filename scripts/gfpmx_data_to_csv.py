...
See also the notebook that explores this data.
...
# Output folder
gfpmx_data_dir = gftmx_data_dir + "gfpmx"
...
gfpmx_data['IndroundProd'].to_csv(data_dir + "/" + "IndroundProd.csv")

# Write all sheets to csv files
for key in tqdm(gfpmx_data.keys()):
    gfpmx_data[key].to_csv(gfpmx_data_dir + "/" + key + ".csv", index=False)
